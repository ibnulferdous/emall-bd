...
const loadProducts = () => {
  ...
  // Show products on initial loading
  showProducts(data);
};
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    // Width for rating stars
    const ratingPercentage = (product.rating.rate / 5) * 100;
    // Round to nearest 10
    const width = `${Math.round(ratingPercentage / 10) * 10}%`;
    ...
    div.innerHTML = `
      <div class="single-product h-100 px-3 py-4 d-flex flex-column justify-content-between">
        <div>
          ...
          <p class="text-black-50 mb-1">Category: ${product.category}</p>
          <div>
            <div class="stars-outer">
              <div id="stars-inner-${product.id}" class="stars-inner"></div>
            </div>
            <p class="text-black-50">${product.rating.rate}(${product.rating.count})</p>
          </div>
          <h2 class="h5 fw-bold mb-3 text-muted">Price: $ ${product.price.toFixed(2)}</h2>
        </div>
        <div>
    ...
    document.getElementById(`stars-inner-${product.id}`).style.width = width;
  }
};

// Function to update on "Add to Cart" button click
let count = 0;
